...
**Reader Note**. Much of the text in this section
is repetitive of previous section (\@ref(hcc-5hmcseq-model-suite}).
Rework wording later.
...
estimated probability of LumA for sample i and $y_i$ is 1 for LumA samples and
0 for Others.  ie. we use the fitted sample contribution to the
likelihood function as a sample *consistency* score,
which is $P(LumA)$, the predicted probabilaiy of LumA for LumA samples
and $1 - P(LumA)$ for Others.  To derive the sample *consistency scores,
...
The variability in sample set *consistency* the we get from simple random sampling,
...
**Reader Note** 
``` 
We still need to domonstrate the effect of sample set composition
on results.  With random sampling the effects area subtle and not
immediately obvious.  We may have to fabricate sample sets with 
low consistency scores - ie. made up of many hard to classify smaples -
in order to show this inpact.
```
## Full data set fit
...
```{r get-all-data, cache=T, cache.vars=c('all_geneExpr_mtx', 'all_group_vec')}

# combine train and test 
all_geneExpr_mtx <- rbind(train_geneExpr_mtx, test_geneExpr_mtx)
...
```{r lasso-fit-all, cache=T, cache.vars=c('BrCa_cv_lassoAll_lst'), eval=F}

set.seed(1)

start_time <-  proc.time()

BrCa_cv_lassoAll_lst <- lapply(1:30, function(REP) {
glmnet::cv.glmnet(
 x = all_geneExpr_mtx,
 y = factor(all_group_vec,levels = c('Other', 'LumA')),
 alpha = 1,
 family = 'binomial',
 type.measure  =  "class",
 keep = T,
...
<!-- DO THIS ONCE -->
```{r save-BrCa_cv_lassoAll_lst, cache=T, dependson='lasso-fit-all', cache.vars='', echo=F, eval=F}
 save(list='BrCa_cv_lassoAll_lst', file=file.path("RData",'BrCa_cv_lassoAll_lst'))
```
```{r load-BrCa_cv_lassoAll_lst, cache=F, echo=T}
 load(file=file.path("RData",'BrCa_cv_lassoAll_lst'))
```
...
### CLEAR CACHE
plot(
 log(BrCa_cv_lassoAll_lst[[1]]$lambda),
 BrCa_cv_lassoAll_lst[[1]]$cvm,
 lwd=2,
 xlab='log(Lambda)', ylab='CV Misclassification Error', type='l', ylim=c(0, .5)
)

for(JJ in 2:length(BrCa_cv_lassoAll_lst))
 lines(
  log(BrCa_cv_lassoAll_lst[[JJ]]$lambda),
  BrCa_cv_lassoAll_lst[[JJ]]$cvm,
  lwd=2
)
...
# nzero
nzero_1se_vec <- sapply(BrCa_cv_lassoAll_lst,
 function(cv_fit) cv_fit$nzero[cv_fit$lambda == cv_fit$lambda.1se])

nzero_min_vec <- sapply(BrCa_cv_lassoAll_lst,
...
# predicted probs - 1se
lassoAll_predResp_1se_mtx <- sapply(BrCa_cv_lassoAll_lst, function(cv_fit) { 
...
# predicted probs - min
lassoAll_predResp_min_mtx <- sapply(BrCa_cv_lassoAll_lst, function(cv_fit) { 
...
plot(
 ...
 col = ifelse(all_group_vec == 'LumA', 'red', 'green')
)
 
# Add reference lines at 10% false positive
thres_1se <- quantile(lassoAll_predResp_1se_vec[all_group_vec == 'Other'], prob=.9)
thres_min <- quantile(lassoAll_predResp_min_vec[all_group_vec == 'Other'], prob=.9)
...
y <- as.numeric(all_group_vec == 'LumA')
# 1se
...
```{r plot-qual-conf, cache=F, cache.vars='', dependson='get-sample-pred', fig.height=5, fig.width=8, fig.cap='consistency scores by classification - Other=0, LumA=1'}
...
other as an LimA case), the algorithm is *less certain* of its predicted
outcome than for the fals negative cases (1:0 or classifying an LimA case as a other).
ie. the misclassified LumA samples are quite similar to Others, whereas there
is more ambiguity in the misclassified Other samples.
...
# 1se
lassoAll_coef_1se_lst <- lapply(BrCa_cv_lassoAll_lst, function(cv_fit){
 cv_fit_coef <- coef(
 cv_fit,
 s = "lambda.1se"
 )
...
# min
lassoAll_coef_min_lst <- lapply(BrCa_cv_lassoAll_lst, function(cv_fit){
 cv_fit_coef <- coef(
 cv_fit,
 s = "lambda.min"
 )
...
there could be significant time effects.  For example, the first 25 LumA samples could come
from Center A, while the next 25 could come from Center B.  And 
LimA and other samples could be acquired from different clinics
...
batch effects that are associated with class labels - others being in
different batches than LimA samples is an all too common occurence,
...
selected sets of LumA samples is $`r round(100*(300/sum(all_group_vec=='LumA'))^2,1)`$%.
For Others the expected overlap is $`r round(100*(300/sum(all_group_vec=='Other'))^2,1)`$%. 
...
```{r get-all-vec, cache=T, cache.vars=c('all_other_vec', 'all_LimA_vec')}

all_other_vec <- names(all_group_vec[all_group_vec=='Other']) 
all_LimA_vec <- names(all_group_vec[all_group_vec=='LumA'])  

```

We have $`r length(all_other_vec)`$ other sample IDs stored in `all_other_vec`
and $`r length(all_LimA_vec)`$ LimA sample IDs stored in `all_LimA_vec`.
To create a suite of random samples from these, we only need to randomly select indices from
each vector.

  
```{r getSimTable, cache=T, cache.vars=c('sim_other_mtx', 'sim_LimA_mtx')}

set.seed(12379)

sim_other_mtx <- sapply(
 1:SIM, 
 function(dummy) 
   sample(1:length(all_other_vec), size =  max(SIZE))
)
...
sim_LimA_mtx <- sapply(
 1:SIM, 
 function(dummy) 
   sample(1:length(all_LimA_vec), size =  max(SIZE))
)
...
`sim_other_mtx` and `sim_LimA_mtx`, each with domensions $`r dim(sim_LimA_mtx)`$.
...
```{r look-sim-qual_ARCHIVED, cache=T, cache.vars=c('sim_other_qual_mtx', 'sim_LimA_qual_mtx'), fig.height=8, fig.width=10, fig.cap='sample consistency by simulation run', eval=F, echo=F}
### CLEAR CACHE

all_other_qual_vec <- sample_qual_vec[all_other_vec]
sim_other_qual_mtx <- sapply(
  1:ncol(sim_other_mtx), 
  function(CC) all_other_qual_vec[sim_other_mtx[,CC]]
 )

all_LimA_qual_vec <- sample_qual_vec[all_LimA_vec]
sim_LimA_qual_mtx <- sapply(
  1:ncol(sim_LimA_mtx),  
  function(CC) all_LimA_qual_vec[sim_LimA_mtx[,CC]]
 )
...
sim_LimA_qual_byStage_lst <- do.call('c', 
 lapply(1:ncol(sim_LimA_qual_mtx), 
  function(CC) c(split(sim_LimA_qual_mtx[,CC], stage_vec),NA)
 )
)
...
# LimA
boxplot(
  sim_LimA_qual_byStage_lst, 
  outline = F, 
  border = 1:6,
  ylab = 'Quality Score',
  xaxt = 'n'
)
sim_ndx <- which(names(sim_LimA_qual_byStage_lst)=='')
abline(v = which(names(sim_LimA_qual_byStage_lst)==''), col = 'grey')
axis(
  side=1, 
  at = sim_ndx-2,        
  label = 1:length(sim_ndx),
...
```{r look-sim-qual-0-50ONLY, cache=T, cache.vars=c('sim_other_qual_mtx', 'sim_LimA_qual_mtx'), fig.height=8, fig.width=10, fig.cap='sample consistency by simulation run for size = 50 '}
### CLEAR CACHE
...
30 simulations, or random selections of other and LimA samples.
We see significant variability in sample consistency, especially in the LimA cases.
...
for (SIMno in 1:ncol(sim_other_qual_mtx)) {
  ...
  brca_sim_cv_lst <- lapply(1:length(levels(stage_vec)), function(STGno) {
    ...
    sim_stage_samples_vec <- c(
      all_other_vec[sim_other_mtx[Stage_rows_vec, SIMno]],
      all_LimA_vec[sim_LimA_mtx[Stage_rows_vec, SIMno]]
    )
    sim_stage_geneExpr_mtx <- all_geneExpr_mtx[sim_stage_samples_vec, ]
    ...
    sim_stage_cv_lst <- lapply(1:CV_REP, function(CV) {
      ...
      # test error
      sim_stage_test_samples_vec <- setdiff(rownames(all_geneExpr_mtx), sim_stage_samples_vec)
      sim_stage_test_geneExpr_mtx <- all_geneExpr_mtx[sim_stage_test_samples_vec,]
      sim_stage_test_group_vec <- all_group_vec[sim_stage_test_samples_vec]

      test_pred_1se_vec <- predict(
       cv_fit,
       newx=sim_stage_test_geneExpr_mtx,
       s="lambda.1se",
       type="class"
      )
  ...
  # save  brca_sim_cv_lst
  fName <- paste0("brca_sim_", SIMno, "_cv_lst")
...
```{r extract-sim-results, cache=T, cache.vars='lasso_brca_sim_results_frm'}
### CLEAR CACHE
brca_sim_files_vec <- list.files('RData', '^brca_sim_')
...
cv_lst_to_frm <- function(brca_brca_sim_cv_lst) {
 do.call('rbind', lapply(1:length(brca_brca_sim_cv_lst),
  function(JJ) {
    siz_frm <- cvList2frm_f(brca_brca_sim_cv_lst[[JJ]])
    data.frame(Size=SIZE[JJ], siz_frm)
  }))
}

# we loop across simulations to combine all results into one big table
lasso_brca_sim_results_frm <- do.call('rbind', lapply(1:length(brca_sim_files_vec),
 function(SIM_NO) {
  load(file=file.path('RData', brca_sim_files_vec[SIM_NO]))
  assign('brca_brca_sim_cv_lst', get(brca_sim_files_vec[SIM_NO]))
  rm(list=brca_sim_files_vec[SIM_NO])
  
  data.frame(SimNo=paste0('Sim_',formatC(SIM_NO,width = 2,flag = 0)), cv_lst_to_frm(brca_brca_sim_cv_lst))
} 
)) 
...
<!-- 
Have a table of simulation results - `lasso_brca_sim_results_frm`:
-->

```{r sum-table, cache=T, cache.vars='', fig.cap='Simution results table'}
### CLEAR CACHE
 
knitr::kable(head(with(lasso_brca_sim_results_frm, table(SimNo, Size))),
  caption = paste("Simulation Results - N Sim =", SIM)) %>%
   kableExtra::kable_styling(full_width = F)

knitr::kable(head(lasso_brca_sim_results_frm) %>% dplyr::select(-c(genes_1se, genes_min)),
...
stage_vec <- cut(1:nrow(brca_sim_other_qual_mtx), c(0,SIZE), include.lowest = T)


#SIM <- "Sim_01"

for(SIM in unique(lasso_brca_sim_results_frm$SimNo)[1]){

SimNum <- as.numeric(sub('Sim_','',SIM))

simNo_results_frm <- lasso_brca_sim_results_frm %>% dplyr::filter(SimNo==SIM)
...
SKIP  <- function() {
# Add qual annotation
other_qual_vec <- sapply(split(brca_sim_other_qual_mtx[,SimNum], stage_vec), median)
LimA_qual_vec <- sapply(split(brca_sim_LimA_qual_mtx[,SimNum], stage_vec), median)
LL <- LL + 1
axis(side=1, tick=F, line = LL,
  at = match(paste0(SIZE0,'_cv'),names(error_1se_lst)),
...
#########################################
## cv
cv_1se_Bysize_lst <- lapply(unique(lasso_brca_sim_results_frm$Size),
function(SizeVal) {
 sizeVal_results_frm <- lasso_brca_sim_results_frm %>% dplyr::filter(Size==SizeVal)
...
names(cv_1se_Bysize_lst) <- paste0(
 stringr::str_pad(unique(lasso_brca_sim_results_frm$Size), width=3, pad='0'), '_cv')

## test
test_1se_Bysize_lst <- lapply(unique(lasso_brca_sim_results_frm$Size),
function(SizeVal) {
 sizeVal_results_frm <- lasso_brca_sim_results_frm %>% dplyr::filter(Size==SizeVal)
...
names(test_1se_Bysize_lst) <- paste0(
 stringr::str_pad(unique(lasso_brca_sim_results_frm$Size), width=3, pad='0'), '_test')
...
#########################################
## cv
cv_min_Bysize_lst <- lapply(unique(lasso_brca_sim_results_frm$Size),
function(SizeVal) {
 sizeVal_results_frm <- lasso_brca_sim_results_frm %>% dplyr::filter(Size==SizeVal)
...
## test
test_min_Bysize_lst <- lapply(unique(lasso_brca_sim_results_frm$Size),
function(SizeVal) {
 sizeVal_results_frm <- lasso_brca_sim_results_frm %>% dplyr::filter(Size==SizeVal)
...
#########################################
# selected features
p_1se_Bysize_lst <- lapply(unique(lasso_brca_sim_results_frm$Size),
function(SizeVal) {
 sizeVal_results_frm <- lasso_brca_sim_results_frm %>% dplyr::filter(Size==SizeVal)
...
names(p_1se_Bysize_lst) <- paste0(
 stringr::str_pad(unique(lasso_brca_sim_results_frm$Size), width=3, pad='0'), '_p')

# selected signatue features
sign_p_1se_Bysize_lst <- lapply(unique(lasso_brca_sim_results_frm$Size),
function(SizeVal) {
 sizeVal_results_frm <- lasso_brca_sim_results_frm %>% dplyr::filter(Size==SizeVal)
...
names(sign_p_1se_Bysize_lst) <- paste0(
 stringr::str_pad(unique(lasso_brca_sim_results_frm$Size), width=3, pad='0'), '_signP')
...
#########################################
# selected features
p_min_Bysize_lst <- lapply(unique(lasso_brca_sim_results_frm$Size),
function(SizeVal) {
 sizeVal_results_frm <- lasso_brca_sim_results_frm %>% dplyr::filter(Size==SizeVal)
...
# selected signatue features
sign_p_min_Bysize_lst <- lapply(unique(lasso_brca_sim_results_frm$Size),
function(SizeVal) {
 sizeVal_results_frm <- lasso_brca_sim_results_frm %>% dplyr::filter(Size==SizeVal)
...
```{r small-sample-qual, cache=T, cache.vars=c(''), fig.cap='sample consistency vs classifier performance'}

other_samp25_qual_vec <- apply(brca_sim_other_qual_mtx[1:25, ], 2, median)
LimA_samp25_qual_vec <- apply(brca_sim_LimA_qual_mtx[1:25, ], 2, median)
...
LimA sample cohorts does have a measurable impact on the 1se cv and test set error
