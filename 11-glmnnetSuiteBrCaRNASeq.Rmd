...
cv_lst_to_frm <- function(brca_sim_cv_lst) {
 do.call('rbind', lapply(1:length(brca_sim_cv_lst),
  function(JJ) {
    siz_frm <- cvList2frm_f(brca_sim_cv_lst[[JJ]])
...
lasso_brca_sim_results_frm <- do.call('rbind', lapply(1:length(brca_sim_files_vec),
 function(SIM_NO) {
  ...
  assign('brca_sim_cv_lst', get(brca_sim_files_vec[SIM_NO]))
  rm(list=brca_sim_files_vec[SIM_NO])
  
  data.frame(SimNo=paste0('Sim_',formatC(SIM_NO,width = 2,flag = 0)), cv_lst_to_frm(brca_sim_cv_lst))
} 
)) 
