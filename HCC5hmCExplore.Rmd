...
which is an essential tool for anyone who is doing predicitive analytics in R - `caret` R pacakge.
...
# 5hmc Data Preprocessing {#preproc}
...
the more parsinomious *1se* models in the ridge and elastice net case.
...
estimates of misclassification error rates are closer to the
...
genomic scale data sets of moderate size.
It has also been our experience that the cv estimates of 
misclassification rates become more biased as the sample size decreases.
We will explore this question in Section \@ref(model-suite).  
-->
...
```{r lookLassoR, cache=T, dependson='fitLassoR', cache.vars='', fig.height=5, fig.width=6, fig.cap="Relaxed lasso fit", echo=T}
...
```{r lookLassoR2,cache=T, dependson='fitLassoR', cache.vars=''}
# only report  1se
...
```{r confMtxTrainLasso, cache=F, cache.vars='', fig.cap="Train set confusion", echo=F, eval=F}
...
<!--chapter:end:04-glmnetFits.Rmd-->

# Does Size Matter? {#model-suite}
...
```{r lasso-fit-all, cache=T, cache.vars=c('cv_lassoAll_lst')}
...
```{r plot-qual-conf, cache=F, cache.vars='', dependson='get-sample-pred', fig.height=5, fig.width=8, fig.cap='quality scores by classification - HCC=1'}
...
gplots::boxplot2(split(sample_1se_qual_vec, lassoAll_1se_conf_vec), 
  outline=F, ylab = 'Quality Score', ylim=c(0,1))
title('1se Model')
...
```{r feature-list-1se, cache=T, cache.vars=c('genes_by_rep_1se_tbl','lassoAll_coef_1se_mtx'), fig.height=5, fig.width=8, fig.cap="Feature list stability for one the standard error rule models"}
...
gene_sign_1se_vec <- rownames(lassoAll_coef_1se_mtx)[rowSums(lassoAll_coef_1se_mtx)==30]
gene_sign_min_vec <- rownames(lassoAll_coef_min_mtx)[rowSums(lassoAll_coef_min_mtx)==30]

```

`r length(intersect(gene_sign_1se_vec, gene_sign_min_vec))` out of
`r length(gene_sign_1se_vec)` of the genes in the 1se model gene signature
...
for(SIM_NO in 1:length(sim_files_vec)) {
  ...
  tmp <- sim_cv_lst
  for(L1 in 1:length(sim_cv_lst))
   for(L2 in 1:length(sim_cv_lst[[L1]]))
   { tmp[[L1]][[L2]]$oof_1se <- NULL
     tmp[[L1]][[L2]]$oof_min <- NULL
    }

  sim_cv_lst <- tmp
  assign(sim_files_vec[SIM_NO], sim_cv_lst)
  save(list=sim_files_vec[SIM_NO], file=file.path("RData",sim_files_vec[SIM_NO]))
...
```{r sim-time, cache=T, dependson='run-sim', eval=F}
...
```{r extract-sim-results, cache=T, cache.vars='sim_results_frm'}
...
### Simulation Results:  Errors By Run {-}

```{r simRes-errors-bySim, cache=T, cache.vars='what-to-keep', fig.heigth=5, fig.width=10, fig.cap='Errors by Sample Size and Quality'}
...
for(SIM in unique(sim_results_frm$SimNo)){
...
### Simulation Results:  Model Size By Run {-}

```{r simRes-features-bySim, cache=T, cache.vars='what-to-keep', fig.heigth=5, fig.width=10, fig.cap='Selected Features by Sample Size and Quality'}

# get full model nzero ref
nzero_1se_vec <- sapply(cv_lassoAll_lst,
 function(cv_fit) cv_fit$nzero[cv_fit$lambda == cv_fit$lambda.1se])
nzero_1se_q1q3 <- quantile(nzero_1se_vec, prob=c(1,3)/4)

nzero_min_vec <- sapply(cv_lassoAll_lst,
 function(cv_fit) cv_fit$nzero[cv_fit$lambda == cv_fit$lambda.min])
nzero_min_q1q3 <- quantile(nzero_min_vec, prob=c(1,3)/4)
...
# get selected features that are part of gene_sign_1se_vec
# - the signature selected genes
sign_genes_1se_lst <- lapply(1:nrow(simNo_results_frm), function(RR)
    intersect(unlist(simNo_results_frm[RR, 'genes_1se']), gene_sign_1se_vec))
...
abline(h= nzero_1se_q1q3, col = 'red')
legend('topright',
   #title='1se errors', title.col = 'black',
   text.col = c('blue', 'green'),
...
# get selected features that are part of gene_sign_min_vec
# - the signature selected genes
sign_genes_min_lst <- lapply(1:nrow(simNo_results_frm), function(RR)
    intersect(unlist(simNo_results_frm[RR, 'genes_min']), gene_sign_min_vec))
...
abline(h= nzero_min_q1q3, col = 'red')
legend('topright',
   #title='min errors', title.col = 'black',
   text.col = c('blue', 'green'),
   legend= c('selected genes','signature genes'),
...
<!--chapter:end:05-glmnetSuite.Rmd-->
...
<!-- net-fit-all takes a while - save results -->
<!-- DO THIS ONCE -->
...
```
```{r load-cv_enetAll_lst, cache=T, dependson='enet-fit-all', cache.vars='', echo=F}
...
```{r enet-get-sample-qual, cache=T, dependson='get-sample-pred', cache.vars=c('sample_1se_qual_vec','sample_min_qual_vec')}
# get qual scores

y <- as.numeric(all_group_vec == 'HCC')
# 1se
p <- enetAll_predResp_1se_vec
sample_1se_qual_vec <- p^y*(1-p)^(1-y)

# min
p <- enetAll_predResp_min_vec
sample_min_qual_vec <- p^y*(1-p)^(1-y)
...
```{r enet-plot-qual-conf, cache=F, cache.vars='', dependson='get-sample-pred', fig.height=5, fig.width=8, fig.cap='quality scores by classification - HCC=1'}

y <- as.numeric(all_group_vec == 'HCC')

# 1se
enetAll_1se_conf_vec <- paste(
 y, 
 as.numeric(enetAll_predClass_1se_vec=='HCC'),
 sep = ':'
)

# min
enetAll_min_conf_vec <- paste(
 y, 
 as.numeric(enetAll_predClass_min_vec=='HCC'),
 sep = ':'
)


tmp <- c(
 split(sample_1se_qual_vec, enetAll_1se_conf_vec), 
 split(sample_min_qual_vec, enetAll_min_conf_vec)
)

par(mfrow=c(1,2), oma=c(2,0,2,0))
gplots::boxplot2(split(sample_1se_qual_vec, enetAll_1se_conf_vec), 
  outline=F, ylab = 'Quality Score', ylim=c(0,1))
title('1se Model')

gplots::boxplot2(split(sample_min_qual_vec, enetAll_min_conf_vec), 
  outline=F, ylab = 'Quality Score', ylim=c(0,1))
title('min lambda Model')
...
gene_sign_1se_vec <- rownames(enetAll_coef_1se_mtx)[rowSums(enetAll_coef_1se_mtx)==30]
gene_sign_min_vec <- rownames(enetAll_coef_min_mtx)[rowSums(enetAll_coef_min_mtx)==30]
...
For a given simulation and a given sample size, we will obtain
CV_REP = $`r CV_REP`$ cross-validated enet fits.  From these fits,
we can obtain $`r CV_REP`$ out-of-fold assessments of classification accuracy 
to get a sense if its variability. From each cv replicate, we also obtain
an estimated model size and a set of selected features.  We will want
to examine how these stabilize as the sample size increases.
...
```{r enet-get-all-vec, cache=T, cache.vars=c('all_control_vec', 'all_affected_vec')}
...
```{r enet-getSimTable, cache=T, cache.vars=c('sim_control_mtx', 'sim_affected_mtx')}
...
```{r enet-look-sim-qual, cache=T, cache.vars=c('sim_control_qual_mtx', 'sim_affected_qual_mtx'), fig.height=8, fig.width=10, fig.cap='sample quality by simulation run'}
...
<!-- RUN ONCE - THEN GET FROM MEMORY -->
```{r enet-run-sim, cache=T, cache.vars='start_time', eval=T}
start_time <- proc.time()

# Get stage from SIZE
stage_vec <- cut(1:nrow(sim_control_qual_mtx), c(0, SIZE), include.lowest = T)

# ran in two runs 1:5, 6:ncol
for (SIMno in 6:ncol(sim_control_qual_mtx)) {
...
```{r enet-DEBUG, eval=F, echo=F}
### REMOVE oof errors.  cv errors are the same!
sim_files_vec <- list.files('RData', '^enet_sim_')
...
```{r enet-simRes-errors-bySim, cache=T, cache.vars='what-to-keep', fig.heigth=5, fig.width=10, fig.cap='Errors by Sample Size and Quality'}
# CLEAR CACHE 
...
```{r enet-simRes-features-bySim, cache=T, cache.vars='what-to-keep', fig.heigth=5, fig.width=10, fig.cap='Selected Features by Sample Size and Quality'}
# CLEAR CACHE 
...
<!--chapter:end:05B-glmnetSuiteB.Rmd-->
...
<!--chapter:end:06-variableImportance.Rmd-->
