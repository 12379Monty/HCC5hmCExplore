...
dotted reference line in Figure \@ref(fig:hcc5hmC-glmnetFit-lookLassoR)).
...
See Table \@ref(tab:hcc5hmC-glmnetFit-printErrors).  
...
on the right side of Figure \@ref(fig:hcc5hmC-glmnetSuite-get-sample-pred).  
...
We saw in Table \@ref(tab:hcc5hmC-glmnetSuite-model-size-lassoAll) that the number of features
...
### Results across simulation runs.
...
dotted reference line in Figure \@ref(fig:brcaRna-glmnetFit-lookLassoR)).
This feature is pointed out in the 
[glmnet 3.0 vignette](https://cran.r-project.org/web/packages/glmnet/vignettes/relax.pdf):
*The debiasing will potentially improve prediction performance,
and CV will typically select a model with a smaller number of variables.*
...
```{r brcaRna-glmnetFit-heatmapLasso, cache=T, cache.vars='', fig.height=6, fig.width=8, fig.cap="Lasso Model Genes", echo=F, eval=F}
### CLEAR CACHE
 suppressPackageStartupMessages(require(gplots))
...
```{r brcaRna-glmnetFit-heatmapEnet, cache=T, cache.vars='', fig.height=6, fig.width=8, fig.cap="Enet Model Genes", echo=F, eval=F}
### CLEAR CACHE
 suppressPackageStartupMessages(require(gplots))
...
See Table \@ref(tab:brcaRna-glmnetFit-printErrors).  
...
the thresholds to achieve a fixed false positive rate, 10% say.
These thresholds are indicated by the grey lines in the scatter plot
on the right side of Figure \@ref(fig:brca-rnaseq-get-sample-pred).  
...
We saw in Table \@ref(tab:brca-rnaseq-model-size-lassoAll) that the number of features
selected by the minimum lambda models had an IQR of
`r paste(quantile(nzero_min_vec,1/4), quantile(nzero_min_vec,3/4), sep='-')`,
while the one standard error rule models had an IQR of
`r paste(quantile(nzero_1se_vec,1/4), quantile(nzero_1se_vec,3/4), sep='-')`.
