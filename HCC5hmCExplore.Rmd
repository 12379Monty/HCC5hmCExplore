...
```{r estBCV, cache=T, cache.vars=c('BCV_mtx','BCV_90perc_vec', 'BCV_50perc_vec'), fig.height=5, fig.width=10, fig.cap="Cumulative Distribution of CV - rug = 90th percentile"}
 ...
 # Plot
 plot(CDF(density(BCV_mtx[,1])), col=groupCol[colnames(BCV_mtx)[1]], lwd=2, ylab='Prob(BCV<x)',
   xlim=c(0, 0.5))
 for(JJ in 2:ncol(BCV_mtx))
 plot(CDF(density(BCV_mtx[,JJ])), col=groupCol[colnames(BCV_mtx)[JJ]], lwd=2, add=T, xlim=c(0, 2.0))
...
```{r sampleSizeCurve, cache=T, cache.vars='', fig.height=8, fig.width=11,fig.cap='Sample Size Estimates'}
 ...
 for(EFFECT in c(1.25, 1.10)){
  ...
  plot(x=1:50, y= RNASeqPower::rnapower(depth=1:50, cv=1.2, effect=EFFECT, alpha=.05, power=.80), 
      type='n', lwd=2, ylim=c(0,ifelse(EFFECT==2, 50, 250)), ylab="", xlab="")
 
  for(GRP in names(BCV_90perc_vec))
  lines(x=1:50, y= RNASeqPower::rnapower(depth=1:50, cv=BCV_90perc_vec[GRP], effect=EFFECT, alpha=.05, power=.80),
  ...
  legend('topright', legend=paste(names(BCV_90perc_vec), 'CV =', round(BCV_90perc_vec, 2)), 
