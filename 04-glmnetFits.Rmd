...
```{r lookLasso, cache=T, cache.vars='', fig.height=5, fig.width=5, fig.cap="lasso fit"}
 lambda.1se_p <- sum(abs(coef(cv_lasso, s = cv_lasso$lambda.1se))>0)
 lambda.min_p <- sum(abs(coef(cv_lasso, s = cv_lasso$lambda.min))>0)

plot(
 cv_lasso, 
 sub=paste("1se p =", lambda.1se_p, ";min p =", lambda.min_p)
)
...
plot(
 ...
 cv_lasso$cvm,
 pch=16,col="red",
 xlab="log(Lambda)",ylab=cv_lasso$name)
