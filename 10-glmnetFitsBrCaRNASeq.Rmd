...
is repetitive of previous section (\@ref(hcc-5hmcseq-explore-sparsity).
...
```{r brca-rnaseseq-getTrainVal, cache=T, cache.vars=c('train_sampID_vec', 'test_sampID_vec','train_group_vec','test_group_vec','train_geneExpr_mtx','test_geneExpr_mtx')}

### CLEAR CACHE

set.seed(1)
train_sampID_vec <- with(sampDescB,
sampDescB$sampID[caret::createDataPartition(y=group, p=trainP, list=F)]
)

test_sampID_vec <- with(sampDescB,
setdiff(sampID, train_sampID_vec)
)

train_group_vec <- factor(sampDescB[train_sampID_vec, 'group'],
levels=c('Other', 'LumA'))
names(train_group_vec) <- sampDescB[train_sampID_vec, 'sampID']

test_group_vec <- factor(sampDescB[test_sampID_vec, 'group'],
levels=c('Other', 'LumA'))
names(test_group_vec) <- sampDescB[test_sampID_vec, 'sampID']
...
train_geneExpr_mtx <- t(geneExpressionB_F[,train_sampID_vec])
test_geneExpr_mtx <- t(geneExpressionB_F[,test_sampID_vec])
...
* How sparse are models enabling good 5hmC classification of LumA vs Other subpype samples?    
...
```{r brca-rnaseseq-getTrainFolds, cache=T, cache.vars='train_foldid_vec'}
...
Suppose our response variable Y is either 0 or 1 (LumA or Other in our case).
...
class predictions extracted by invoking `predict()` with paramters `newx = train_data`
...
```{r brca-rnaseseq-doMC, include=F}
...
```{r brca-rnaseseq-fit-lasso, cache=T, cache.vars=c('cv_lasso')}
### CLEAR CACHE

start_time <-  proc.time()

cv_lasso <- glmnet::cv.glmnet(
 x=train_geneExpr_mtx,
 y=train_group_vec,
...
```{r brca-rnaseseq-fit-ridge, cache=T, cache.vars=c('cv_ridge')}
### CLEAR CACHE
...
```{r brca-rnaseseq-fit-enet, cache=T, cache.vars=c('cv_enet')}
### CLEAR CACHE
...
```{r brca-rnaseseq-fit-lassoC, cache=T, cache.vars=c('cv_lassoC'), eval=F, echo=F}
### CLEAR CACHE
start_time <-  proc.time()
...
```{r brca-rnaseseq-plot_cv_f,echo=T}
...
```{r brca-rnaseseq-lookFits, cache=F, cache.vars='', fig.height=5, fig.width=11, fig.cap="compare fits", echo=T, warnings=F,message=F}
...
```{r brca-rnaseseq-printErrors, fig.cap='model errors'}
...
```{r brca-rnaseseq-fitLassoR, cache=T, cache.vars=c('cv_lassoR'), include=F}
### CLEAR CACHE
...
```{r brca-rnaseseq-lookLassoR, cache=T, dependson='fitLassoR', cache.vars='', fig.height=5, fig.width=6, fig.cap="Relaxed lasso fit", echo=T}
### CLEAR CACHE
...
```{r brca-rnaseseq-lookLassoR2,cache=T, dependson='fitLassoR', cache.vars=''}
### CLEAR CACHE
# only report  1se
...
train_oofPred_relaxed_1se_vec <- ifelse(
  logistic_f(cv_lassoR$fit.preval[["g:0"]][, ndx_1se]) > 0.5, "LumA", "Other"
)
...
train_oofPred_blended_1se_vec <- ifelse(
  logistic_f(cv_lassoR$fit.preval[["g:0.5"]][, ndx_1se]) > 0.5, "LumA", "Other"
)
...
```{r brca-rnaseseq-trainROC, cache=F, cache.vars='', fig.height=5, fig.width=5, fig.cap="Train data out-of-sample ROCs"}
### CLEAR CACHE
### CLEAR CACHE
...
train_lasso_roc <- pROC::roc(
 response = as.numeric(train_group_vec=='LumA'),
...
```{r brca-rnaseseq-thresh90, cache=F, cache.vars='', fig.cap='90% Specificity Thresholds'}
### CLEAR CACHE
...
```{r brca-rnaseseq-trainOOFprops, cache=F, cache.vars='', fig.height=8, fig.width=10, fig.cap="Train data out-of-fold predicted probabilities"}
### CLEAR CACHE

par(mfrow = c(2, 2), mar = c(3, 3, 2, 1), oma = c(2, 2, 2, 2))

# lasso
plot(density(train_lasso_oofProb_vec[train_group_vec == "Other"]),
  xlim = c(0, 1), main = "", xlab = "", ylab = "", col = "green"
)
lines(density(train_lasso_oofProb_vec[train_group_vec == "LumA"]),
  col = "red"
)
title("lasso")
...
# enet
plot(density(train_enet_oofProb_vec[train_group_vec == "Other"]),
  xlim = c(0, 1), main = "", xlab = "", ylab = "", col = "green"
)
lines(density(train_enet_oofProb_vec[train_group_vec == "LumA"]),
  col = "red"
)
title("enet")

# lassoR
plot(density(train_relaxed_oofProb_vec[train_group_vec == "Other"]),
  xlim = c(0, 1), main = "", xlab = "", ylab = "", col = "green"
)
lines(density(train_relaxed_oofProb_vec[train_group_vec == "LumA"]),
  col = "red"
)
title("lassoR")

# blended
plot(density(train_blended_oofProb_vec[train_group_vec == "Other"]),
  xlim = c(0, 1), main = "", xlab = "", ylab = "", col = "green"
)
lines(density(train_blended_oofProb_vec[train_group_vec == "LumA"]),
  col = "red"
)
title("blended")
...
```{r brca-rnaseseq-testROC, cache=F, cache.vars='', fig.height=5, fig.width=5, fig.cap="Test data out-of-sample ROCs", echo=T, include=F}
### CLEAR CACHE
...
test_lasso_roc <- pROC::roc(
  response = as.numeric(test_group_vec == "LumA"),
...
```{r brca-rnaseseq-testROC2, cache=F, cache.vars='', fig.height=5, fig.width=5, fig.cap="Test data out-of-sample ROCs", echo=T, include=T}
### CLEAR CACHE
### CLEAR CACHE
# plot all
...
```{r brca-rnaseseq-testOOFprobs, cache=F, cache.vars='', fig.height=8, fig.width=10, fig.cap="Test data out-of-fold predicted probabilities", echo=T}
### CLEAR CACHE

par(mfrow = c(2, 2), mar = c(3, 3, 2, 1), oma = c(2, 2, 2, 2))

# lasso
plot(density(test_lasso_predProb_vec[test_group_vec == "Other"]),
  xlim = c(0, 1), main = "", xlab = "", ylab = "", col = "green"
)
lines(density(test_lasso_predProb_vec[test_group_vec == "LumA"]),
  col = "red"
)
title("lasso")
legend("topright", legend = c("Other", "LumA"), text.col = c("green", "red"))
...
# enet
plot(density(test_enet_predProb_vec[test_group_vec == "Other"]),
  xlim = c(0, 1), main = "", xlab = "", ylab = "", col = "green"
)
lines(density(test_enet_predProb_vec[test_group_vec == "LumA"]),
  col = "red"
)
title("enet")

# relaxed
plot(density(test_relaxed_predProb_vec[test_group_vec == "Other"]),
  xlim = c(0, 1), main = "", xlab = "", ylab = "", col = "green"
)
lines(density(test_relaxed_predProb_vec[test_group_vec == "LumA"]),
  col = "red"
)
title("relaxed")
...
# blended
plot(density(test_blended_predProb_vec[test_group_vec == "Other"]),
  xlim = c(0, 1), main = "", xlab = "", ylab = "", col = "green"
)
lines(density(test_blended_predProb_vec[test_group_vec == "LumA"]),
  col = "red"
)
title("blended")
...
```{r brca-rnaseseq-fitPrevalByGroup, cache=F, cache.vars='', fig.height=8, fig.width=8,fig.cap="Predicted Probabilities - Train and Test",echo=T}
### CLEAR CACHE
...
```{r brca-rnaseseq-trainLassoPred, cache=F, cache.vras='',fig.height=5, fig.width=11,fig.cap="train data lassofit", eval=F, echo=F}
### CLEAR CACHE
...
plot(
 ...
 pch=ifelse(train_group_vec == '_Other', 1, 4),
 xlim=c(-5,5)
 )  
...
```{r brca-rnaseseq-confMtxTrainLasso, cache=F, cache.vars='', fig.cap="Train set confusion", echo=F, eval=F}
### CLEAR CACHE
...
# COMPARE THIS
mean(train_lasso_predClass_vec != train_group_vec)
#[1] 0.03655108
...
all_models_confustion_mtx <- rbind(
 train_lasso  = as.vector(table(train_lasso_predClass_vec, train_group_vec)),
 ...
 train_enet  = as.vector(table(train_enet_predClass_vec, train_group_vec)),
 ...
 train_relaxed  = as.vector(table(train_relaxed_predClass_vec, train_group_vec)),
 ...
 train_blended  = as.vector(table(train_blended_predClass_vec, train_group_vec)),
...
```{r brca-rnaseseq-misclassTrain, cache=F, cache.vars='', fig.height=5, fig.width=8, fig.cap="out-of-fold predicted probabilities at miscassified samples"}
### CLEAR CACHE
...
points(
 ...
 col=ifelse(train_group_vec[rownames(missclass_oofProb_mtx)[RR]] == '_Other',
  'green', 'red'),
...
```{r brca-rnaseseq-misclassTest, cache=F, cache.vars='', fig.height=5, fig.width=8, fig.cap="Test data predicted probabilities at miscassified samples"}
### CLEAR CACHE
...
points(
 rep(RR, ncol(missclass_oofProb_mtx)), 
 missclass_oofProb_mtx[RR,],
 col=ifelse(test_group_vec[rownames(missclass_oofProb_mtx)[RR]] == '_Other',
  'green', 'red'),
 pch=1:ncol(missclass_oofProb_mtx))
...
```{r brca-rnaseseq-compCoeffProf, cache=F, cache.vars='', fig.height=6, fig.width=8, fig.cap="Coefficient Profiles"}
### CLEAR CACHE
...
```{r brca-rnaseseq-zreros, fig.cap=''}
### CLEAR CACHE
...
```{r brca-rnaseseq-pairsCoeffProf, cache=F, cache.vars='', fig.height=6, fig.width=8, fig.cap="Coefficients from fits"}
### CLEAR CACHE
...
```{r brca-rnaseseq-heatmapLasso, cache=T, cache.vars='', fig.height=6, fig.width=8, fig.cap="Lasso Model Genes"}
### CLEAR CACHE
...
lasso_coef_frm <- data.frame(
 ...
 lasso=lasso_coef@x)
  ...
  heatmap.2(
    x=t(train_geneExpr_mtx[,lasso_coef_frm$gene[-1]]),
    scale="row",
    ...
    key=F,
    ColSideColors=ifelse(train_group_vec=='_Other', 'green','red'),
    dendrogram="both",
    main=paste('lasso genes - N =', nrow(lasso_coef_frm)-1))

```

```{r brca-rnaseseq-heatmapEnet, cache=T, cache.vars='', fig.height=6, fig.width=8, fig.cap="Enet Model Genes"}
### CLEAR CACHE
 suppressPackageStartupMessages(require(gplots))
...
enet_coef_frm <- data.frame(
 ...
 enet=enet_coef@x)
  ...
  Mycol <- colorpanel(1000, "blue", "red")
  heatmap.2(
    x=t(train_geneExpr_mtx[,enet_coef_frm$gene[-1]]),
    scale="row",
