...
```{r preprocBreast-setup-preproc, include=F}
...
```{r preprocBreast-chunk-options, include=FALSE, eval=F}
...
<!-- Add base libraries -->
```{r preprocBreast-libraries, include=FALSE, eval=T}
...
```{r preprocBreast-loadData, cache=F}
### CLEAR CACHE
...
This choice is predicted on the desire to have a fairly
...
```{r preprocBreast-subsetSamplesB, cache=T, cache.vars=c('sampDescB','groupCol'), echo=T}


ER_HER2_tbl <- with(sampDesc %>% 
...
# Use suffix 'B' to distinguish from sampDescA 
sampDescB <-
  sampDesc %>%
...
#with(sampDescB, table(group, exclude=NULL))

# Recode group
sampDescB$group <- with(
  sampDescB,
  ifelse(group == "LumA", "LumA", "Other")  ## Luma > Other
)

# Re-order and name rows
o.v <- with(sampDescB, order(group, sampID))
sampDescB  <- sampDescB[o.v, ]

rownames(sampDescB) <- sampDescB$sampID

# set groupCol for later
groupCol <- c("#F3C300", "#875692")
names(groupCol) <- unique(sampDescB$group)
...
Assemble feature matrix for `sampDescB` samples.

```{r preprocBreast-getfeatures, cache=T, cache.vars=c('genes_annotB','geneExpressionB'), echo=T}


# Start for geneExpression_repl1 data
repl1_ndx <- which(is.element(colnames(geneExpression_repl1), sampDescB$sampID))
geneExpressionB <- geneExpression_repl1[, repl1_ndx]

# add samples from geneExpression_subJJ
for(JJ in 1:5) {
  sub_ndx <- which(is.element(
   colnames(get(paste0('geneExpression_sub',JJ))),  sampDescB$sampID))
  geneExpressionB <- cbind(geneExpressionB,
   get(paste0('geneExpression_sub',JJ))[, sub_ndx])
}

# align geneExpressionB columns to sampDescB samples order
geneExpr_ndx <- match(sampDescB$sampID, colnames(geneExpressionB))
if(any(is.na(geneExpr_ndx)))
stop("sampDescB/geneExpressionB: sample mismatch") else
geneExpressionB <- geneExpressionB[, rownames(sampDescB)]

# Rename genes_annot
genes_annotB <- genes_annot
...
```{r preprocBreast-geneExprBxp, cache=T, fig.height=4, fig.width=10, fig.cap='Sample log2 count boxplots', echo=T}


par(mar = c(1, 3, 2, 3))
boxplot(geneExpressionB,
  ...
  border = groupCol[sampDescB$group]
)
legend("top", legend = names(groupCol), text.col = groupCol, 
  ncol = 2, bty = "n")
# Add reference lines
SampleMedian <- apply(geneExpressionB, 2, median)
...
Coverage/geneExpression level looks fairly comparable across samples.  It is sometimes helpful to
keep track of the actual coverage which can be adequetely tracked by distribution
quantiles.
-->

```{r preprocBreast-quantCountsA, echo=F, eval=F}


geneExpressionB_quant <- apply(geneExpressionB, 2, function(CC) {
  c(quantile(CC, prob = c(.15, (1:3) / 4)), totCovM = sum(CC) / 1e6)
})

geneExpressionB_quant2 <- apply(geneExpressionB_quant, 1, function(RR) {
  quantile(RR, prob = (1:3) / 4)
})

knitr::kable(geneExpressionB_quant2,
  digits = 1,
  caption = paste(
...
$TICKr round(geneExpressionB_quant2["75%", "totCovM"],1)TICK$M reads, 25% of samples
have a 15 percentile of coverage lower than
$TICKr geneExpressionB_quant2["25%", "15%"]TICK$, etc.  
-->
...
```{r preprocBreast-rlr, cache=T, cache.vars='lcpm_mtx', fig.height=4, fig.width=10, fig.cap='Sample RLR', eval=F, echo=F,include=F}

### PLACE-HOLDER
lcpm_mtx <- edgeR::cpm(geneExpressionB, log = T)
...
# Add group Q1, Q3
for (GRP in unique(sampDescB$group)) {
  group_ndx <- which(sampDescB$group == GRP)
...
We will adapt this pipeline - basically skip the 2nd and 3rd step.
...
```{r preprocBreast-densityGeneExpr, fig.height=4, fig.width=10, fig.cap='Sample gene expression densities', eval=T, echo=T}


par(mar = c(4, 3, 2, 1))
plot(density(geneExpressionB[, 1]), 
  col = groupCol[sampDescB$group[1]], lwd = 2, 
  ylim = c(0, .30), xlim=c(-5, 10),
  las = 2, main = "", xlab = "gene expression"
)
abline(v = 0, col = 3)
# After verifying no outliers, can plot a random subset 
for (JJ in sample(2:ncol(geneExpressionB), size = 100)) {
  den <- density(geneExpressionB[, JJ])
  lines(den$x, den$y, col = groupCol[sampDescB$group[JJ]], lwd = 2)
} # for(JJ
...
`r weak_flg <- rowSums(geneExpressionB > EXPR_THR) < SAMP_THR `
Removing `r round(100 * mean(weak_flg), 1)`%  of genes...

```{r preprocBreast-removeWeak, cache=T, cache.vars=c('geneExpressionB_F', 'genes_annotBF', 'geneExpressionB'), echo=T, include=F}

### CLEAR CACHE

# Use suffix '_F' for Filtered genes
geneExpressionB_F <- geneExpressionB[!weak_flg, ]

genes.ndx <- match(rownames(geneExpressionB_F), genes_annotB$geneSymbol)
if(sum(is.na(genes.ndx))) stop("geneExpressionB/genes_annotB mismatch")
genes_annotBF <- genes_annotB[genes.ndx,]

dim(geneExpressionB_F)
...
```{r preprocBreast-densityGeneExpr2, fig.height=4, fig.width=10, fig.cap='Sample gene expression densities after removing weak genes', eval=T, echo=T}

### CLEAR CACHE

par(mar = c(4, 3, 2, 1))
plot(density(geneExpressionB_F[, 1]),
  col = groupCol[sampDescB$group[1]],
  lwd = 2, las = 2, main = "", xlab = "gene expression",
  ylim = c(0, .30), xlim=c(-5, 10)
)
#abline(v = 0, col = 3)
# After verifying no outliers, can plot a random subset 
for (JJ in sample(2:ncol(geneExpressionB_F), size = 100)) {
  den <- density(geneExpressionB_F[, JJ])
  lines(den$x, den$y, col = groupCol[sampDescB$group[JJ]], lwd = 2)
} # for(JJ
legend("topright", legend = names(groupCol), 
  text.col = groupCol, bty = "n")
...
```{r preprocBreast-getDGEL, cache=T, cache.var=c('AF_dgel', 'AF_lcmp_mtx'), eval=F, echo=F}

### CLEAR CACHE

AF_dgel <- edgeR::DGEList(
  counts = geneExpressionB_F,
  genes = genes_annotBF,
  samples = sampDescB,
  group = sampDescB$group
)
...
```{r preprocBreast-normedLcpmBxp, cache=T, fig.height=4, fig.width=10, fig.cap='Sample log2 count boxplots', echo=F, eval=F}
...
```{r preprocBreast-plotMDS, cache=T, fig.height=5, fig.width=10, fig.cap='MDS plots of gene expression values', echo=T}
...
# remove from final version
set.seed(1)
samp_ndx <- sample(1:ncol(geneExpressionB_F), size = 500)
MDS.out <- limma::plotMDS(geneExpressionB_F[, samp_ndx],
  col = groupCol[sampDescB$group[samp_ndx]], pch = 1
)
legend("topleft",
...
-->
```{r preprocBreast-GlMDSplot, echo=T,cache=T, cache.vars='', fig.height=6, fig.width=11,fig.cap="MDS plots of gene expression values", echo=F, eval=F}
...
```{r preprocBreast-DEADesign, cache=F, include=T, echo=T, include=T}

### CLEAR CACHE

Design_mtx <- model.matrix( ~  -1 + group, data=sampDescB)
...
```{r preprocBreast-printDesign, echo=T, include=F}
...
```{r preprocBreast-Voom1, cache=T, cache.vars=c('filteredCountsAF_voom'), fig.height=6, fig.width=11, fig.cap="Removing heteroscedascity", echo=F, eval=F}

### CLEAR CACHE

par(mfrow=c(1,1))
geneExpressionB_F_voom <- limma::voom(geneExpressionB_F, Design_mtx, plot=T)
...
```{r preprocBreast-lmFit, cache=T, echo=T, cache.vars=c('geneExpressionB_F_efit','geneExpressionB_F_efit_dt', 'geneExpressionB_F_efit'),echo=T}



 geneExpressionB_F_fit <- limma::lmFit(geneExpressionB_F, Design_mtx)
 colnames(geneExpressionB_F_fit$coefficients) <- sub("\\(Intercept\\)", "Intercept",
 colnames(geneExpressionB_F_fit$coefficients) )

 geneExpressionB_F_fit <- limma::contrasts.fit(
    geneExpressionB_F_fit, contrasts=Contrasts_mtx)

 geneExpressionB_F_efit <- limma::eBayes(geneExpressionB_F_fit)

 geneExpressionB_F_efit_dt <-
 limma::decideTests(geneExpressionB_F_efit,adjust.method = "BH", p.value = 0.05)
 
 knitr::kable(t(summary(geneExpressionB_F_efit_dt)),
...
```{r preprocBreast-mdPlotEfit, cache=T, cache.vars='GC_vec', fig.height=6, fig.width=11, fig.cap="LumA vs Other - Genes Identified at FDR = 0,05", echo=T}
...
limma::plotMD(geneExpressionB_F_efit,
 ylim = c(-0.5, 0.5),
 column='LumAvsOther',
 status=geneExpressionB_F_efit_dt[,'LumAvsOther'],
 hl.pch = 16, hl.col = c("lightblue", "pink"), hl.cex = .5,
 bg.pch = 16, bg.col = "grey", bg.cex = 0.5,
 main = '',
 xlab = paste0(
    "Average log-expression: IQR=",
    paste(round(quantile(geneExpressionB_F_efit$Amean, prob = c(1, 3) / 4), 2),
      collapse = ", "
    )
  ),
  ylab = paste0(
    "log-fold-change: IQR=",
    paste(round(quantile(geneExpressionB_F_efit$coefficients[, 'LumAvsOther'], prob = c(1, 3) / 4), 2),
      collapse = ", "
    )
  ),
...
rug(quantile(geneExpressionB_F_efit$coefficients[, 'LumAvsOther'], prob = c(1, 2, 3) / 4),
  col = "purple",
  ticksize = .03, side = 2, lwd = 2
)
rug(quantile(geneExpressionB_F_efit$Amean, prob = c(1, 2, 3) / 4),
  col = "purple",
...
boxplot(split(
 geneExpressionB_F_efit$coefficients[, 'LumAvsOther'],
 geneExpressionB_F_efit_dt[,'LumAvsOther']),
 outline=F,
...
genes_ndx <- match(rownames(geneExpressionB_F_efit), genes_annotBF$geneSymbol)
if(sum(is.na(genes_ndx))) stop("geneExpressionB_F_efit/genes_annotBF: genes mismatch")
GC_vec <- with(genes_annotBF[genes_ndx,],(G+C)/(A+C+G+T))
...
```{r preprocBreast-quantlogFC,echo=T}

###  CLEAR CACHE

geneExpressionB_F_logFC_sum <- sapply(
 split(
 geneExpressionB_F_efit$coefficients[, 'LumAvsOther'],
 geneExpressionB_F_efit_dt[,'LumAvsOther']),
 quantile, prob = (1:3) / 4)

colnames(geneExpressionB_F_logFC_sum) <- as.character(factor(
 colnames(geneExpressionB_F_logFC_sum), 
 levels=c("-1", "0", "1"),
 labels=c('down', 'notDE', 'up')
))


knitr::kable(geneExpressionB_F_logFC_sum,
  digits = 2,
...
\@ref(tab:preprocBreast-quantlogFC) and 
\@ref(tab:quantlogFC).
...
quite small
(TICKr round( 
mean(GC_vec[geneExpressionB_F_efit_dt[,'LumAvsOther']=='-1']) -
mean(GC_vec[geneExpressionB_F_efit_dt[,'LumAvsOther']!='-1']),
3)TICK
...
```{r preprocBreast-mdPlotTfit, cache=T, cache.vars='', fig.height=6, fig.width=11, fig.cap="LumA vs Other - Identified Genes at FDR = 0,05 and logFC > 10%",echo=T}

###  CLEAR CACHE

geneExpressionB_F_tfit <- limma::treat(geneExpressionB_F_fit, lfc=log2(1.10))
geneExpressionB_F_tfit_dt <- limma::decideTests(geneExpressionB_F_tfit)

cat("10% FC Gene Identification Summary - voom, adjust.method = BH, p.value = 0.05:\n")
summary(geneExpressionB_F_tfit_dt)

# log-fold-change vs ave-expr
limma::plotMD(geneExpressionB_F_efit,
 ylim = c(-0.6, 0.6),
 column='LumAvsOther',
 status=geneExpressionB_F_tfit_dt[,'LumAvsOther'],
...
```{r preprocBreast-altCV, cache=T, cache.vars='',fig.height=4, fig.width=6, fig.cap="Alternative CV Calculation", eval=F, echo=F}

###  CLEAR CACHE

fit <- geneExpressionB_F_efit
...
```{r preprocBreast-plotSNR,  fig.height=5, fig.width=10, fig.cap="Cumulative Distribution of SNR - rug = 25, 50, 75 and 90th percentile",message=F, echo=T,include=T}

###  CLEAR CACHE

Effect <- abs(geneExpressionB_F_efit$coefficients[,'LumAvsOther'])
Noise <- geneExpressionB_F_efit$sigma
SNR <- Effect/Noise
