...
par(mfrow=c(2,2), mar=c(5,3,2,1), oma=c(2,2,1,0))

for(VAR in c("p", "cv_error", "oof_error", "test_error")){
   var_lst <- split(sim_cv_frm[, VAR], sim_cv_frm$Size)
   names(var_lst) <- SIZE
...
for(JJ in 1:5) {
  boxplot(sim_cv_frm %>% dplyr::filter(Size==JJ) 
    %>% dplyr::select("cv_error", "oof_error", "test_error"),
    xaxt='n', ylim=c(0,.5))
   axis(side=1, at=1:3, labels=c("cv", "oof", "test"))
