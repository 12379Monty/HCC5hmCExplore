...
```{r lasso-fit-all, cache=T, cache.vars=c('cv_lassoAll.lst')}
...
```{r model-size-lassoAll, cache=F, dependson='lasso-fit-all', cache.vars='', fig.height=5, fig.width=8, fig.cap='Feature selection and estimated error by repeated cv lasso models'}
...
boxplot(list(`1se`=nzero_1se_vec, min = nzero_min_vec), ylab="Number of selected features")
...
rownames(lassoAll_predResp_1se_mtx) <- rownames(all_lcpm_mtx)
...
rownames(lassoAll_predResp_min_mtx) <- rownames(all_lcpm_mtx)
...
# compare the two
plot(
 x = lassoAll_predResp_1se_vec, xlab='one standard error rule models',
 y = lassoAll_predResp_min_vec, ylab='minimum lambda models',
...
gplots::boxplot2(split(sample_1se_qual_vec, lassoAll_1se_conf_vec), ylab = 'Quality Score')
title('1se lambda Q vs C')

gplots::boxplot2(split(sample_min_qual_vec, lassoAll_min_conf_vec), ylab = 'Quality Score')
title('min lambda Q vs C')
...
for (SIMno in 1:ncol(sim_control_qual_mtx)) {
  ...
  sim_cv_lst <- lapply(1:length(levels(stage_vec)), function(STGno) {
    ...
    sim_stage_cv_lst <- lapply(1:CV_REP, function(CV) {
      ...
      # oof error
      oofPred_min_vec <- ifelse(
        cv_fit$fit.preval[, ndx_min] > 0.5, "HCC", "Control"
      )
      oofPred_min_error <- mean(oofPred_min_vec != sim_stage_group_vec)
      ...
      list(
       ...
       cv_1se = cvm_1se, 
       cv_min = cvm_min, 
       oof_1se = oofPred_1se_error, 
       oof_min = oofPred_min_error, 
...
```{r sim-time, cache=T, dependson='run-sim'}
...
```{r simRes-Sim1-1se, cache=T, cache.vars='what-to-keep', fig.heigth=5, fig.width=8, fig.cap='Look at 1se results for Sim 1'}

SIM <- "Sim_01"

for(SIM in unique(sim_results_frm$SimNo)){

somNo_results_frm <- sim_results_frm %>% dplyr::filter(SimNo==SIM)

# 1se
par(mfrow=c(2,2), mar=c(5,3,2,1), oma=c(0,2,2,0))
for(VAR in c("p_1se", "cv_1se", "oof_1se", "test_1se")){
   var_lst <- split(somNo_results_frm[, VAR], somNo_results_frm$Size)
   names(var_lst) <- SIZE
   boxplot(var_lst, main=VAR)
}
mtext(side=3, outer=T, cex=1.25, paste("Sim =", SIM))

# min
par(mfrow=c(2,2), mar=c(5,3,2,1), oma=c(0,2,2,0))
for(VAR in c("p_min", "cv_min", "oof_min", "test_min")){
   var_lst <- split(somNo_results_frm[, VAR], somNo_results_frm$Size)
   names(var_lst) <- SIZE
   boxplot(var_lst, main=VAR)
}
...
# 1se
par(mfrow=c(2,3), mar=c(5,3,2,1), oma=c(0,2,2,0))
for(SZ in SIZE[1:5]) {
  boxplot(somNo_results_frm %>% dplyr::filter(Size==SZ) 
    %>% dplyr::select("cv_1se", "oof_1se", "test_1se"),
    xaxt='n', ylim=c(0,.5))
   axis(side=1, at=1:3, labels=c("cvi_1se", "oof_1se", "test_1se"))
   title(paste('Size =', SZ))
}
mtext(side=3, outer=T, cex=1.25, paste("Sim =", SIM))
...
for(SZ in SIZE[1:5]) {
  boxplot(somNo_results_frm %>% dplyr::filter(Size==SZ) 
    %>% dplyr::select("cv_min", "oof_min", "test_min"),
    xaxt='n', ylim=c(0,.5))
   axis(side=1, at=1:3, labels=c("cvi_min", "oof_min", "test_min"))
   title(paste('Size =', SZ))
}
mtext(side=3, outer=T, cex=1.25, paste("Sim =", SIM))
