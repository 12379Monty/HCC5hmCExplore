...
```{r lookFits, cache=F, cache.vars='', fig.height=5, fig.width=11, fig.cap="compare fits", echo=T}
...
```{r lookLassoR, cache=T, cache.vars='', fig.height=5, fig.width=5, fig.cap="lassoR fit", echo=T}
...
knitr::kable(t(data.frame(
  train_lassoR_cv = cv_lassoR_1se_error,
...
continue to be good indicators of unseen data error rates.
...
distribution - predicted probabilities arr very close to 0 or 1.
...
# lassoR
test_lassoR_predProb_vec <- predict(
  cv_lassoR,
  type = "resp",
  lambda = "1se",
  newx = test_lcpm_mtx,
...
test_lassoR_roc <- pROC::roc(
  response = as.numeric(test_group_vec == "HCC"),
  predictor = test_lassoR_predProb_vec
)
