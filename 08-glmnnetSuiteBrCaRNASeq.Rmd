...
```{r brca-rnaseq-get-all-data, cache=T, cache.vars=c('all_geneExpr_mtx', 'all_group_vec')}
### CLEAR CACHE
...
<!-- DO THIS ONCE -->
```{r brca-rnaseq-save-BrCa_cv_lassoAll_lst, cache=T, dependson='lasso-fit-all', cache.vars='', echo=F, eval=F}
 save(list='BrCa_cv_lassoAll_lst', file=file.path("RData",'BrCa_cv_lassoAll_lst'))
```
```{r brca-rnaseq-load-BrCa_cv_lassoAll_lst, cache=F, echo=T}
...
```{r brca-rnaseq-get-sample-qual, cache=T, dependson='get-sample-pred', cache.vars=c('sample_1se_qual_vec','sample_min_qual_vec')}
# get qual scores
...
```{r brca-rnaseq-get-all-vec, cache=T, cache.vars=c('all_other_vec', 'all_LumA_vec')}

all_other_vec <- names(all_group_vec[all_group_vec=='Other']) 
all_LumA_vec <- names(all_group_vec[all_group_vec=='LumA'])  

```

We have `r length(all_other_vec)` other sample IDs stored in `all_other_vec`
and `r length(all_LumA_vec)` LumA sample IDs stored in `all_LumA_vec`.
To create a suite of random samples from these, we only need to randomly select indices from
each vector.

  
```{r brca-rnaseq-getSimTable, cache=T, cache.vars=c('sim_other_mtx', 'sim_LumA_mtx')}
...
sim_LumA_mtx <- sapply(
 1:SIM, 
 function(dummy) 
   sample(1:length(all_LumA_vec), size =  max(SIZE))
)
...
`sim_other_mtx` and `sim_LumA_mtx`, each with domensions `r dim(sim_LumA_mtx)`.
...
all_LumA_qual_vec <- sample_qual_vec[all_LumA_vec]
sim_LumA_qual_mtx <- sapply(
  1:ncol(sim_LumA_mtx),  
  function(CC) all_LumA_qual_vec[sim_LumA_mtx[,CC]]
 )
...
```{r brca-rnaseq-look-sim-qual-0-50ONLY, cache=T, cache.vars=c('sim_other_qual_mtx', 'sim_LumA_qual_mtx'), fig.height=8, fig.width=10, fig.cap='sample consistency by simulation run for size = 50 '}
### CLEAR CACHE
...
for (SIMno in 1:ncol(sim_other_qual_mtx)) {
  ...
  brca_sim_cv_lst <- lapply(1:length(levels(stage_vec)), function(STGno) {
    ...
    sim_stage_samples_vec <- c(
      ...
      all_LumA_vec[sim_LumA_mtx[Stage_rows_vec, SIMno]]
    )
...
```{r brca-rnaseq-extract-sim-results, cache=T, cache.vars='lasso_brca_sim_results_frm'}
...
SKIP  <- function() {
# Add qual annotation
other_qual_vec <- sapply(split(sim_other_qual_mtx[,SimNum], stage_vec), median)
LumA_qual_vec <- sapply(split(sim_LumA_qual_mtx[,SimNum], stage_vec), median)
LL <- LL + 1
axis(side=1, tick=F, line = LL,
  at = match(paste0(SIZE0,'_cv'),names(error_1se_lst)),
...
other_samp25_qual_vec <- apply(sim_other_qual_mtx[1:25, ], 2, median)
LumA_samp25_qual_vec <- apply(sim_LumA_qual_mtx[1:25, ], 2, median)
