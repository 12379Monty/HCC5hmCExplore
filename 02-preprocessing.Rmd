...
with(sampDescA, knitr::kable(table(group, exclude = NULL),
...
```{r getDGEL, cache=T, cache.var=c('filteredCountsAF_dgel', 'filteredCountsAF_lcmp_mtx')}

filteredCountsAF_dgel <- edgeR::DGEList(
  counts = featureCountsAF,
  genes = genes_annotAF,
  samples = sampDescA,
  group = sampDescA$group
)
filteredCountsAF_dgel <- edgeR::calcNormFactors(filteredCountsAF_dgel)
filteredCountsAF_lcmp_mtx <- edgeR::cpm(filteredCountsAF_dgel, log = T)

# Save filteredCountsAF_dgel to facilitate restarting
# remove from final version
save(list = "filteredCountsAF_dgel", file = "RData/filteredCountsAF_dgel")
```
...
par(mar = c(1, 3, 2, 1))
boxplot(filteredCountsAF_lcmp_mtx,
...
legend("top", legend = names(groupCol), text.col = groupCol,
  ncol = 2, bty = "n")
# Add reference lines
SampleMedian <- apply(filteredCountsAF_lcmp_mtx, 2, median)
abline(h = median(SampleMedian), col = "grey")
axis(side = 4, at = round(median(SampleMedian), 1),
  las = 2, col = "grey", line = -1, tick = F)
...
# remove from final version
set.seed(1)
samp_ndx <- sample(1:ncol(filteredCountsAF_lcmp_mtx), size = 500)
MDS.out <- limma::plotMDS(filteredCountsAF_lcmp_mtx[, samp_ndx],
...
Glimma::glMDSPlot(filteredCountsAF_dgel[, samp_ndx],
  groups = filteredCountsAF_dgel$samples[
    samp_ndx,
...
Design_mtx <- model.matrix( ~  -1 + group, data=filteredCountsAF_dgel$samples)
...
par(mfrow=c(1,1))
filteredCountsAF_voom <- limma::voom(filteredCountsAF_dgel, Design_mtx, plot=T)
...
```{r lmFit, cache=T, echo=T, cache.vars=c('filteredCountsAF_voom_efit','filteredCountsAF_voom_efit_dt'),echo=F}
 ...
 knitr::kable(summary(filteredCountsAF_voom_efit_dt),
  caption="DE Results ad FDR = 0.05")

```

### Graphical representations of differential expression results: MD Plots {-}
...
### Number of DE genes at 10% fold change
...
For the filtered reads, coverage looks liek this:
