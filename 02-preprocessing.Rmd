...
## Normalized representation feature - $log_2$ CPM
...
# this is simply a matter of convenience and to save computing time
 ...
 legend("topleft", legend = names(groupCol), text.col = groupCol, bty = "n")
 MDS.out <- limma::plotMDS(lcpm_mtx[,samp_ndx], col=groupCol[sampDescA$group[samp_ndx]], pch=1,
     dim.plot=3:4)
...
```{r estBCV, cache=T, cache.vars=c('BCV_mtx','BCV_90perc_vec', 'BCV_50perc_vec'), fig.height=5, fig.width=10, fig.cap="Cumulative Distribution of CV - rug = 90th percentile"}
 ...
 function(GRP) {
  ...
  GRP_dgel <- suppressMessages(edgeR::estimateDisp(GRP_dgel))
...
## REDO THIS - just one curve at the 90perc CV.
## or two curves - one for 90per and one for 50 perc
## elaborate
...
Note that moderate sample sizes are adequte to detect genes with an effect size
of 1.25 ...
